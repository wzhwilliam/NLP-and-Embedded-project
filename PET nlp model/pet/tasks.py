...
class BoolMProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir: str) -> List[InputExample]:
        return self._create_examples(os.path.join(data_dir, BoolMProcessor.TRAIN_FILE_NAME), "train")

    def get_dev_examples(self, data_dir: str) -> List[InputExample]:
        return self._create_examples(os.path.join(data_dir, BoolMProcessor.DEV_FILE_NAME), "test")

    def get_test_examples(self, data_dir) -> List[InputExample]:
        return self._create_examples(os.path.join(data_dir, BoolMProcessor.TEST_FILE_NAME), "test")

    def get_unlabeled_examples(self, data_dir) -> List[InputExample]:
        return self._create_examples(os.path.join(data_dir, BoolMProcessor.UNLABELED_FILE_NAME), "unlabeled")

    def get_labels(self) -> List[str]:
        return BoolMProcessor.LABELS
    ...
    def _create_examples(self, path, set_type, max_examples=-1, skip_first=0):
        ...
        with open(path, encoding='utf8') as f:
            ...
            for idx, row in enumerate(reader):
                ...
                label = [int(x) for x in row[BoolMProcessor.LABEL_COLUMNS[0]:BoolMProcessor.LABEL_COLUMNS[1] + 1]]
                text_a = row[BoolMProcessor.TEXT_A_COLUMN]
                text_b = row[BoolMProcessor.TEXT_B_COLUMN] if BoolMProcessor.TEXT_B_COLUMN >= 0 else None
                example = InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label)
                examples.append(example)
...
TASK_HELPERS = {
    ...
    "boolm": task_helpers.BoolMTaskHelper,
